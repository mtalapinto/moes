    ...
    while ord_red < ord_blu + 1:
        ...
        while k <= wav_N:
            for l in range(len(slitgrid)):
                ...
                DC = np.zeros([3])
                DC[2] = -1.
                single_element = (ord_red, wav_min, H[0], H[1], H[2], DC[0], DC[1], DC[2])
                spectrum.append(np.array(single_element))
            wav_min += dwav
            k += 1
        # file.write('\n')
...
def init_stellar_doppler(rv, fcam):
    slitgrid = []
    diam = 0.1
    decx = 0.0
    decy = 0.0
    ...
    stardir = '/home/eduspec/Documentos/moes/P_s5700g4.50z0.0t0.97_a0.00c0.00n0.00o0.00_VIS.spec.flat/'
    stellar_spec = pd.read_csv(stardir + 'P_s5700g4.50z0.0t0.97_a0.00c0.00n0.00o0.00_VIS.spec',
...
def get_ws_order(date):
    basedir = 'ws\\'
    wsdir = "".join([basedir, str(date), '\\'])
    fib = '2F'
    wsa = pd.read_pickle("".join([wsdir, 'ws_', str(fib), '_1.pkl']))
    wsadata = pd.DataFrame()
    wsadata['wave'] = wsa['wave']*1e-4
    wsadata['x'] = wsa['x']
    wmin_hcl = min(wsa['wave'])
    wmax_hcl = max(wsa['wave'])
    ...
    spectrum = init()
    specmoes = fideos_spectrograph.raytrace(spectrum, det)
    maxorder = max(specmoes['order'])
    minorder = min(specmoes['order'])
    maxwave = max(specmoes['wave'])
    minwave = min(specmoes['wave'])
    outdata = pd.DataFrame()
    while minorder <= maxorder:
        order = specmoes.loc[specmoes['order'] == minorder]
        #print(order)
        wmin_order = min(order['wave'])
        wmax_order = max(order['wave'])
        hcl_lines = wsadata.loc[wsadata['wave'] <= wmax_order]
        hcl_lines = hcl_lines.loc[hcl_lines['wave'] >= wmin_order]
        if len(hcl_lines) != 0:
            #print(hcl_lines)
            outdata_aux = pd.DataFrame()
            outdata_aux['order'] = np.full(len(hcl_lines), minorder)
            outdata_aux['wave'] = hcl_lines['wave'].values
            outdata_aux['x'] = hcl_lines['x'].values
            print(outdata_aux)
            outdata = pd.concat([outdata, outdata_aux], ignore_index=True)

        minorder += 1

    print(outdata)
    outdata = outdata.sort_values(by=['order', 'wave'])
    #outdata = outdata.sort_values(by=['order'])
    print(outdata)
    basedir = 'ws/'
    outdir = "".join([basedir, date, '/'])
    outfile = "".join([outdir, 'ws_', str(fib), '_final.pkl'])
...
if __name__ == '__main__':
    ...
    #init_stellar()
    #init_stellar_doppler(0, 300)
    #init_stellar_doppler_order(-7500, 230, 67)
    #stellarpath = '/home/eduspec/Documentos/moes/fideos_moes/stellar_spectrum/'
    #spectrum = pd.read_csv(stellarpath+'stellar_slit_spec_alt_0.dat',sep=',')
    #specmoes2 = fideos_spectrograph.tracing_full(spectrum)
    #specmoes2.to_csv(stellarpath+'moes_stellar_spectrum_alt_0.csv', index=False)

    #norm_spectra()

    #print(spec)
    #plt.plot(spec['x'], spec['y'],'k.', s=4)
    #plt.show()
