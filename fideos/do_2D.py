...
def create_moes_spectra(rv, o, det):
    order = echelle_orders.init_stellar_doppler_simple(rv, o)
    spec = fideos_spectrograph.raytrace(order, det)
    #print(spec['y'])
    print('moes ray tracing... done')
    spec = spec.loc[spec['x'] >= 0]
    spec = spec.loc[spec['x'] <= det[2]]
    spec = spec.loc[spec['y'] >= 0]
    spec = spec.loc[spec['y'] <= det[3]]
    ...
    outdir = "".join(['/data/matala/luthien/fideos/data/ns0/ccd_', str(det[-1]), '/', str(rv), '/'])
...
def do_all_2D_moes_simple(n):
    ...
    fcol = 762.
    slit = 100
    pixarray = np.arange(7.5, 25.51, 1.5)
    #pixarray = np.arange(22.5, 25.51, 1.5)
    #pixarray = [22.5]
    basedir = "".join(['/data/matala/luthien/fideos/data/ns0/'])
    ...
    print('FIDEOS MOES spectra creation')
    i = 0
    for pixsize in pixarray:
        samp = fcam / fcol * slit / pixsize
        x_pix = x_um / pixsize
        y_pix = y_um / pixsize
        det = [samp, pixsize, x_pix, y_pix, n]
        ...
        if int(i) == int(n):
            for rv in rvs:
                ...
                if not os.path.exists(rvoutdir):
                    os.mkdir(rvoutdir)
                while omin <= omax:
                    create_moes_spectra(rv, omin, det)
                    omin += 1
        #print(pixsize)
        i += 1

def create_all_noisy_data(ns):
    rvs = np.arange(-10000, 10001, 100)
    dets = np.arange(0, 13, 1)
    print(dets)
...
def create_noisy_data(det, rv, order, sn):
    ...
    #wini = 3800
    #wend = 6800
    basedir = '/data/matala/luthien/fideos/data/'
...
if __name__ == '__main__':
    ...
    #do_2D_all(230)
    #do_2D_v2(0, 300)
    create_all_noisy_data(0.01)
    create_all_noisy_data(0.05)
    create_all_noisy_data(0.1)
