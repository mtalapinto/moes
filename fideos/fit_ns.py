...
def carmenes_vis(date, fiber):
    par_ini = parameters.load_sa(fiber)
    ...
    def loglike(cube, ndim, nparams):
        ...
        ndata = len(y)
        loglikelihood = -0.5 * ndata * np.log(2. * np.pi * sigma_fit_x ** 2).sum() + (-0.5 * ((model[:, 2] - y[:, 3]) / sigma_fit_x) ** 2).sum()
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    out_file = 'ns_moes/'+date+'_carm_vis_'+fiber
    
    # Run MultiNest:
    pymultinest.run(loglike, prior, n_params, n_live_points=1000, outputfiles_basename=out_file, resume=False, verbose=False)
...
def nzp_res_dd_offset():
    ...
    def loglike(cube, ndim, nparams):
        ...
        sigma_fit_y = nzp_err
        ndata = len(nzp)
        loglikelihood = -0.5 * ndata * np.log(2. * np.pi * sigma_fit_y ** 2).sum() + (-0.5 * ((nzp - model) / sigma_fit_y) ** 2).sum()
        return loglikelihood
    ...
    # Run MultiNest:
    pymultinest.run(loglike, prior, n_params, n_live_points=5000, outputfiles_basename=out_file, resume=True, verbose=False)
