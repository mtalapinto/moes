...
def ccf(rv, detno, typ, sn):
    ...
    ccf = []
    rv_array = []
    obsdir = '/data/matala/luthien/feros/data/pix_exp/ns' + str(sn) + '/ccd_' + str(detno) + '/' + str(rv) + '/'
    ...
    ccfdir = '/data/matala/luthien/feros/data/ccf/'
...
def ccf_gauss_fit_drv(rv, detno, typ, drv, sn):
    datadir = '/data/matala/luthien/feros/data/ccf/ns' + str(int(sn))
    if typ == 'simple':
...
def do_all_ccf(typ, det):
    ...
    ccfpath = '/data/matala/luthien/feros/data/ccf/'
    if not os.path.exists(ccfpath):
        os.mkdir(ccfpath)
    ...
    for rv in rvarray:
        print(rv)
        if typ == 'simple':
            ccffile = snpath + 'ccd_' + str(det) + '/ccf_' + str(rv) + '_simple.tsv'
        elif typ == 'full':
            ccffile = snpath + 'ccf_' + str(rv) + '_full.tsv'
        elif typ == 'conv':
            ccffile = snpath + 'ccf_' + str(rv) + '_conv.tsv'
...
def get_det_samp(i, sn):
    if sn == 0:
        datadir = 'data/pix_exp/ns'+str(int(sn))+'/ccd_'+str(i)+'/0/'
        orand = 40
...
def plot_ccfs():
    ...
    if not os.path.exists(outdir):
        os.mkdir(outdir)
    for rv in rvs:
        file = basedir+'ccd_'+str(ccd)+'/ccf_'+str(int(rv))+'_simple.tsv'
        data = pd.read_csv(file, sep=',')
        plt.clf()
        plt.plot(data['rv'], data['ccf'], 'k-')
        plt.savefig(outdir+str(int(rv))+'_ccf.png')
        plt.clf()
        plt.close()
        print('plotting RV ', rv)


def plot_spectra():
    det = 0
    rv = -10000
    basedir = 'data/pix_exp/ns0/ccd_'+str(int(det))+'/'+str(rv)+'/'
    order = 48
    maskfile = pd.read_csv('stellar_template/g2mask_new.tsv', names=['wini', 'wend'])
    ones = np.full(len(maskfile), 1)
    orderfile = pd.read_csv(basedir+str(int(order))+'_2D_moes.tsv', ',')
    starfile = pd.read_csv('stellar_template/stellar_template_resampled.tsv', ',')
    plt.plot(orderfile['wave']*1e4, orderfile['flux'], 'k-', alpha=0.6)
    plt.plot(starfile['wave'], starfile['flux'], 'r-', alpha=0.6)
    plt.plot(maskfile['wini'], ones, 'bo')
    plt.plot(maskfile['wend'], ones, 'bo')
    plt.xlim(min(orderfile['wave'])*1e4, max(orderfile['wave'])*1e4)
...
def do_histo_drv(sn):
    ...
    outdir = '/data/matala/moes/feros/results/ns' + str(sn) + '/'
    if not os.path.exists(outdir):
        os.mkdir(outdir)
    print(ndets)
    for i in range(ndets):
        ...
        rvdiff, samp = [], []
        for rv in rvs:
            drvs = np.arange(2.5, 13.5, 0.25)
            drvout = []
            for drv in drvs:
                drvout.append(ccf_gauss_fit_drv(rv, i, 'simple', drv, sn))
            rvdiff.append(min(drvout))
            samp.append(sampling.sampling_maps_det('feros', i))
...
if __name__ == '__main__':
    ...
    #test = pd.read_csv('/home/marcelo/Documentos/moes/feros/data/ccf/ns0/ccd_4/ccf_1000_simple.tsv', sep=',')
    #plt.plot(test['rv'], test['ccf'], 'k-')
    #plt.show()
