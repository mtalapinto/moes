...
def sampling_maps_det(instr, i):
    ...
    det = [sampori, pixsize, npix, npix, i]
...
def sampling_maps_det_full(instr, i):
    rv = 0
    if instr == 'platospec':
        omin = 68
        omax = 122
    ...
    imccd = slit * fcam / fcol
    rv = 0
    idet = pd.read_csv('/data/matala/luthien/'+str(instr)+'/data/pix_exp/ns0_full/ccd_'+str(int(i))+'/0/'+str(int(omin))+'_2D_moes.tsv', sep=',')
    npix = max(idet['pix'])
...
def sampling_maps_det_plot(instr, i, axs):
    rv = 0
    if instr == 'platospec':
        omin = 68
        omax = 122
    ...
    imccd = slit * fcam / fcol
    rv = 0
    idet = pd.read_csv('/data/matala/luthien/' + str(instr) + '/data/pix_exp/ns0/ccd_' + str(int(i)) + '/0/' + str(
        int(omin)) + '_2D_moes.tsv', sep=',')
    npix = max(idet['pix'])
    ...
    x, y = [], []
    while omin <= omax:
        slitout = echelle_orders.init_points_doppler_sampling(rv, instr, omin)
        specout = platospec_moes.tracing_full_det(slitout, det)
        # specout = specout.loc[specout['x'] < 2048]
        # specout = specout.loc[specout['y'] < 2048]
        # specout = specout.loc[specout['x'] > 0]
    ...
    #binwidth = (max(sampout) - min(sampout)) / 25.
    bmax = 3.8
    bmin = 1.
    binwidth = (bmax - bmin) / 30.
    #bins_all = np.arange(min(sampout), max(sampout), binwidth)
    bins_all = np.arange(bmin, bmax, binwidth)

    nsamps, bins, patch_samp = axs['hist'].hist(sampout, color=c2, alpha=0.4, orientation='horizontal', bins=bins_all,
                                                edgecolor='black') #, label=r'Pixel size = ' + str(np.round(pixsize,1)) + r' $\mu$m')
    samp_hist, samp_bins = np.histogram(sampout, bins=30)
    bins_hist_samp = samp_bins[1:] - (samp_bins[2] - samp_bins[1]) / 2
    samp_w_mean = np.sum(bins_hist_samp * (samp_hist / len(sampout)) )

    nsamps, bins, patch_samp = axs['hist'].hist(sampout, color=c2, alpha=0.4, orientation='horizontal', bins=bins_all,
                                                edgecolor='black') #, label='s = '+ str(np.round(sampling, 2)) + ' pix')

    #print(sampori, sampling, samp_w_mean)
    axs['maps'].plot(waveout, sampout, '.', color=c2,
             label=r'Pixel size = ' + str(np.round(pixsize,1)) + r' $\mu$m')
    xarr = [min(waveout) - 0.05, max(waveout) + 0.05]
    #yarr = [sampori, sampori]
    #axs['maps'].plot(xarr, yarr, '--', color=c3)
    yarr = [sampling, sampling]
    xarraux = [0, 1000.]
    xxsize = 6
    axs['maps'].plot(xarr, yarr, '--', color=c3, markersize=xxsize, zorder=20)#, label=r's$ = ' + str(np.round(sampling, 2)) + r' pix')
    axs['hist'].plot(xarraux, yarr, '--', color=c3, markersize=xxsize, zorder=20, label=r's = ' + str(np.round(sampling, 2)) + r' pix')
    yarr = [samp_w_mean, samp_w_mean]
    axs['maps'].plot(xarr, yarr, '--', color=c1, markersize=xxsize, zorder=20)#, label=r's$_{w}$ = ' + str(np.round(samp_w_mean, 2)) + r' pix')
    axs['hist'].plot(xarraux, yarr, '--', color=c1, markersize=xxsize, zorder=20, label=r's$_{w}$ = ' + str(np.round(samp_w_mean, 2)) + r' pix')

    axs['maps'].legend()
    axs['maps'].set_ylim(bmin, bmax)
    axs['hist'].set_ylim(bmin, bmax)
...
if __name__ == '__main__':
    ...
    fig, axs = plt.subplot_mosaic([['maps', 'hist']], figsize=[12, 6], constrained_layout=True, gridspec_kw={'width_ratios': [3,1]})

    #ax1.plot(x, y)
    #ax2.plot(x, -y)
    instr = 'platospec'
    for d in dets:
        sampling_maps_det_plot('platospec', d, axs)
    #plt.xlabel(r'$s$ (pix)')
    #plt.ylabel(r'$\sigma_{RV}$ (m/s)')
    axs['maps'].set_ylabel(r'$s$ (pix)')
    axs['maps'].set_xlabel(r'$\lambda$ ($\mu m$)')
    axs['maps'].set_xlim(0.37, 0.706)
    axs['hist'].set_xlabel(r'N')
    axs['hist'].set_xlim(0, 510)
    axs['hist'].yaxis.tick_right()
    axs['hist'].yaxis.set_label_position("right")
    plt.title('PLATOSpec')
    axs['hist'].legend(loc='best')
    plt.tight_layout()
    plt.savefig('results/plots/'+instr+'_sampling_maps_dets.png')
    plt.show()
    #plt.show()
