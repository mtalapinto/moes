...
def tracing(H, DC, Tin, l0, material, apex, decx, decy):
    ...
    d_prm1_prm2 = np.full(len(H), 30.)
    H_out = trace.to_next_surface(H_out, DC_out, d_prm1_prm2)
    H_out[:, 2] = 0.

    # Coordinate break into prism 2
    decx2 = 19.
    decy2 = 0.0
    H_out[:, 0] = H_out[:, 0] - decx2
    H_out[:, 1] = H_out[:, 1] - decy2

    inter_prisms_angle = 64.4
    T_sf_out = np.array([0, inter_prisms_angle * np.pi / 180, 0])
    DC_out = transform.transform(DC_out, -T_sf_out)
    H_out = transform.transform(H_out, -T_sf_out)
    H_out[:, 0] = H_out[:, 0] - (DC_out[:, 0] / DC_out[:, 2]) * (H_out[:, 2])
    H_out[:, 1] = H_out[:, 1] - (DC_out[:, 1] / DC_out[:, 2]) * (H_out[:, 2])
    ...
    # end coordinate break into prism 2
    ...
    # End prism 2 input surface

    # Coordinate break inter prism surfaces
    decx2 = -6.0
    decy2 = 0.0
    H_out[:, 0] = H_out[:, 0] - decx2
    H_out[:, 1] = H_out[:, 1] - decy2
    ...
    # End coord break inter prism surfaces

    # Prism 2 output surface
    k = n2 / n1
    cosi = DC_out[:, 0] * n[:, 0] + DC_out[:, 1] * n[:, 1] + DC_out[:, 2] * n[:, 2]
    sini = np.sqrt(1 - cosi ** 2)
    sinr = sini / k
    ...
    # alles gute bis hier
    prism_out = np.full(len(H), 20.)
    H_out = trace.to_next_surface(H_out, DC_out, prism_thic)
    H_out[:, 2] = 0.

    # Coordinate break output prism 2
    decx2 = 13.0
    decy2 = 0.0
    H_out[:, 0] = H_out[:, 0] - decx2
    H_out[:, 1] = H_out[:, 1] - decy2
    ...
    # Tilt
    out_angle = 33.7
    T_sf_out = np.array([0, -out_angle * np.pi / 180, 0])
    DC_out = transform.transform(DC_out, T_sf_out)
    H_out = transform.transform(H_out, T_sf_out)
