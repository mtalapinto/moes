...
def ccf(rv, detno, typ, sn):
    ...
    ccf = []
    rv_array = []
    obsdir = '/data/matala/luthien/platospec/data/pix_exp/ns' + str(sn) + '_tilt/ccd_' + str(detno) + '/' + str(rv) + '/'
                ...
                for i in range(len(ordmask)):
                    ...
                    if len(linedata) > 0:
                        if typ == 'simple':
                            ...
                            #print(len(obs), max(linedata['pix']))
                            if max(linedata['pix']) < (len(obs)-1):
                                minpixline = min(linedata['pix'])
                                maxpixline = max(linedata['pix'])
                                #print(obs)
                                #print(i, minpixline, maxpixline)
                                pixmin = obs.loc[obs['pix'] == minpixline]['pix'].values[0]
                                pixmax = obs.loc[obs['pix'] == maxpixline]['pix'].values[0]
                                #print(pixmin, pixmax)
                                pixmaskmin = ordwave2pix(maskwmin)
                                pixmaskmax = ordwave2pix(maskwmax)
                                dpixmin = 1 - np.abs(pixmaskmin - pixmin)
                                dpixmax = 1 - np.abs(pixmaskmax - pixmax)
                                fluxmin = obs.loc[obs['pix'] == minpixline]['flux'].values[0]
                                fluxmax = obs.loc[obs['pix'] == maxpixline]['flux'].values[0]
    ...
    ccfdir = '/data/matala/luthien/platospec/data/ccf/'
    snpath = ccfdir+'ns'+str(sn)+'_tilt/'
...
def ccf_gauss_fit_drv(rv, detno, typ, drv, sn, instr):
    if instr == 'carmenes':
        datadir = '/data/matala/luthien/'+str(instr)+'/ccf/ns' + str(int(sn))
    else:
        datadir = '/data/matala/luthien/'+str(instr)+'/data/ccf/'
    ...
    if typ == 'simple':
        ...
        ccf = pd.read_csv(datadir + '/ns' + str(int(sn)) + '_full/ccd_' + str(detno) + '/ccf_' + str(int(rv)) + '_full.tsv', sep=',')
        ccf['ccf_n'] = (ccf['ccf'] - min(ccf['ccf'].values)) / (
                max(ccf['ccf'].values) - min(ccf['ccf'].values))
    elif typ == 'conv':
        ccf = pd.read_csv('data/ccf/ccd_' + str(detno) + '/ccf_' + str(int(rv)) + '_conv.tsv', sep=',')
        ...
        ccf = pd.read_csv(datadir + '/ns' + str(int(sn)) + '_tilt/ccd_' + str(detno) + '/ccf_' + str(int(rv)) + '_tilt.tsv', sep=',')
        ccf['ccf_n'] = (ccf['ccf'] - min(ccf['ccf'].values)) / (
                max(ccf['ccf'].values) - min(ccf['ccf'].values))
    ccfaux = ccf.loc[ccf['rv'] <= drv + rv * 1e-3]
    ccfaux = ccfaux.loc[ccfaux['rv'] >= -drv + rv * 1e-3]
...
def do_all_ccf(typ, det):
    ...
    rvarray = np.arange(8000, 10001, 100)
    #rvarray = [-10000]
    ccfpath = '/data/matala/luthien/platospec/data/ccf/'
    if not os.path.exists(ccfpath):
        os.mkdir(ccfpath)

    sn = int((0.0 * 100))  # 10 %
    snpath = ccfpath+'ns'+str(sn)+'_tilt/'
    if not os.path.exists(snpath):
        os.mkdir(snpath)
    ...
    for rv in rvarray:
        ...
        if typ == 'simple':
            ...
            ccffile = ccfpath + 'ccf_' + str(rv) + '_tilt.tsv'
...
def do_histo_drv(sn):
    datadir = 'data/ccf/ns'+str(int(sn))+'/'
    ndets = len(glob.glob(datadir+'*'))
    outdir = 'results/ns' + str(sn) + '/'
    import plots
    if not os.path.exists(outdir):
        os.mkdir(outdir)
    ...
    for i in range(ndets):
        ...
        for rv in rvs:
            ...
            samp.append(plots.get_det_samp(i, sn))
...
def do_histo_drv_instrument(instr, sn):
    ...
    for i in range(ndets):
        ...
        for rv in rvs:
            ...
            drvout = []
            for drv in drvs:
                drvout.append(ccf_gauss_fit_drv(rv, i, 'simple', drv, sn, instr))
            rvdiff.append(min(drvout))
            samp.append(sampling.sampling_maps_det('platospec', i))
...
def fix_data(det, rv, order):
    omin = 68
    omax = 122
    datafile = '/data/matala/luthien/platospec/data/pix_exp/ns0_full/ccd_'+str(det)+'/' + str(rv) + '/' + str(order) + '_2D_moes.tsv'
    data = pd.read_csv(datafile)
    data['pix'] = data['pix'] + 1
    data.to_csv(datafile, sep=',', index=False)
    print('Data for Detector no. ', det, 'and RV = ', rv, ' m/s, ', 'spectral order = ', order, ' corrected')
    


def fix_all():
    dets = np.arange(0,12, 1)
    #print(dets)
    for det in dets:
        ...
        for rv in rvarray:
            ...
            while omin <= omax:
                print(det, rv, omin)
                fix_data(det, int(rv), omin)
                omin += 1


def do_histo_drv_plot(instr):
    ...
    samp0 = []
    for k in range(n):
        idet = pd.read_csv(pixdir + 'ns0/ccd_'+str(int(k))+'/0/'+str(int(omin))+'_2D_moes.tsv', sep=',')
        npix = max(idet['pix'])
        ...
        drv = pd.read_csv(basedir + 'results/ns0/drv_ccd_'+str(k)+'_simple.tsv', sep=',')
        std0.append(np.std(drv['drv'] - np.mean(drv['drv'])))
        samp0.append(np.median(drv['samp']))
        print(k, pixsize, samp, np.median(drv['samp']))
    ...
    for i in range(n):
        #print(i)
        drv = pd.read_csv(basedir + 'results/ns1/drv_ccd_' + str(i) + '_simple.tsv', sep=',')
        std1.append(np.std(drv['drv'] - np.mean(drv['drv'])))
        samp1.append(np.median(drv['samp']))
    ...
    for i in range(n):
        #print(i)
        drv = pd.read_csv(basedir + 'results/ns5/drv_ccd_' + str(i) + '_simple.tsv', sep=',')
        std2.append(np.std(drv['drv'] - np.mean(drv['drv'])))
        samp2.append(np.median(drv['samp']))
    ...
    for i in range(n):
      # print(i)
        drv = pd.read_csv(basedir + 'results/ns10/drv_ccd_' + str(i) + '_simple.tsv', sep=',')
        std3.append(np.std(drv['drv'] - np.mean(drv['drv'])))
        samp3.append(np.median(drv['samp']))

    plt.plot(samp0, std0, 'ro', label='0 % noise')
    plt.plot(samp1, std1, 'go', label='1 % noise')
    plt.plot(samp2, std2, 'bo', label='5 % noise')
    plt.plot(samp3, std3, 'o', label='10 % noise', c='purple')
    #plt.plot(samp2, std2, 'bo', label='Noise percent = 5 %')
    #plt.plot(samp3, std3, 'o', c='purple', label='Noise percent = 10 %')
    plt.legend()
    plt.title(title)
    plt.xlabel('Spectral sampling (pix)')
    plt.ylabel(r'$\sigma_{RV}$ (m/s)')
    plt.savefig('results/samp_rv_'+str(instr)+'_final.png')
    plt.show()



def do_all_ccf_full(typ, det):
    ...
    sn = int((0.0 * 100))  # 10 %
    snpath = ccfpath+'ns'+str(sn)+'_full/'
    if not os.path.exists(snpath):
        os.mkdir(snpath)
...
def do_histo_drv_instrument_full(instr, sn):
    ...
    for i in range(ndets):
        ...
        for rv in rvs:
            ...
            drvout = []
            for drv in drvs:
                drvout.append(ccf_gauss_fit_drv(rv, i, 'full', drv, sn, instr))
            rvdiff.append(min(drvout))
...
def do_histo_drv_instrument_tilt(instr, sn):
    if instr == 'carmenes':
        datadir = '/data/matala/luthien/'+str(instr)+'/ccf/ns'+str(int(sn))+'_tilt/'
    else:
        datadir = '/data/matala/luthien/'+str(instr)+'/data/ccf/ns'+str(int(sn))+'_tilt/'
    ndets = len(glob.glob(datadir+'*'))
    outdir = '/data/matala/moes/'+str(instr)+'/results/ns' + str(sn) + '_tilt/'
    import plots
    if not os.path.exists(outdir):
        os.mkdir(outdir)
    ...
    for rv in rvs:
        ...
        drvout = []
        for drv in drvs:
            drvout.append(ccf_gauss_fit_drv(rv, 6, 'tilt', drv, sn, instr))
        rvdiff.append(min(drvout))
        samp.append(sampling.sampling_maps_det_full('platospec', 6))

    rvdiffout = pd.DataFrame()
    rvdiffout['drv'] = rvdiff
    rvdiffout['samp'] = samp
    rvdiffout.to_csv(outdir + 'drv_ccd_' + str(6) + '_simple' + '.tsv', index=False, sep=',')
...
if __name__ == '__main__':
    ...
    #histo_diff()
    #
    #do_histo_drv_plot(sys.argv[-1])
    #do_histo_drv_instrument_full('platospec', 0)
    do_histo_drv_instrument_tilt('platospec', 0)
    #drv_full_plot('platospec')
    #do_histo_drv_instrument('platospec', 1)
    #do_histo_drvres_instrument('platospec', 5)
    #do_histo_drv_instrument('platospec', 10)    
    #fix_data(0, 0, 100)
    #do_all_ccf('tilt', 6)
