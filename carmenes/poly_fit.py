...
def function(coeffs, model):
    model = coeffs[0] + \
            coeffs[1] * model['x'].values + \
            coeffs[2] * model['x'].values ** 2 + \
            coeffs[3] * model['x'].values ** 3

    return model


def correct_dyn(ws_data, ws_model, fiber, date):
    x = ws_model['x'].values  # x position in pixels
    y = ws_data['posm'].values - ws_model['x'].values

    def prior(cube):
        cube[0] = utils.transform_uniform(cube[0], -1.e3, 1.e3)
        cube[1] = utils.transform_uniform(cube[1], -1.e3, 1.e3)
        cube[2] = utils.transform_uniform(cube[2], -1.e3, 1.e3)
        cube[3] = utils.transform_uniform(cube[3], -1.e3, 1.e3)
    ...
    n_params = 4
    outdir = 'data/aberrations_coefficients/poly_coefficients_timeseries/'+date+'/'
